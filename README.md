...
服务每 15 分钟检查一次管理会话，二维码超过 6 天时尝试自动刷新。企业微信没有公开的
...
每次检查会请求微信插件详情以保持管理 Cookie 活跃。临时网络或异常响应只记录状态，
不会直接删除 Cookie；刷新二维码确认会话无效时才清除并要求重新扫码。`/admin` 自身的
8 小时登录期限采用访问续期，持续使用管理页时不会因固定计时突然退出。
